    ...
    protected:
        ...
        void wifi_callback(wpa_cli_msgs::msg::Scan::SharedPtr msg) {
            ...
            if (weights_.rows == 0) {
                ...
                for (int i=0;i<weights_.rows;i++) {
                    for (int j=0;j<weights_.cols;j++) {
                        ...
                        float num = x*x + y*y;
                        float sigma = winsize/6.0f; 
                        weights_(i,j) = exp(-num/(sigma*sigma));
                    }
                }
            ...
            for (size_t i=0;i<it->second.size();i++) {
                ...
                cv::Point2i where;
                where.x = (ws.x - info_.origin.position.x)/info_.resolution;
                where.y = (ws.y - info_.origin.position.y)/info_.resolution;
            ...
            for (unsigned int j=0;j<info_.height;j++) {
                for (unsigned int i=0;i<info_.width;i++) {
                    ...
                    float epsilon = 1e-5;
                    if (og_mat(j,i) != OCCUPIED) {
                        float total = numerator(j,i) / denominator(j,i);
                        if (total < epsilon) {
                            og_mat(j,i) = UNKNOWN;
                        } else {
                            og_mat(j,i) = static_cast<uint8_t>(total);
                        }
                    }
